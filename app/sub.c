...
int main()
{
  ...
  if (GPIOExport(WARNING) == -1 || GPIOExport(MESSAGE) == -1)
    return -1;
  if (GPIODirection(WARNING, IN) == -1 || GPIODirection(MESSAGE, OUT) == -1)
    return -1;

  int prev_state = 0;              // 이전 버튼 상태 (0: 안 눌림, 1: 눌림)
  int curr_state = 0;              // 현재 버튼 상태 (0: 안 눌림, 1: 눌림)
  
  // LED 초기 상태는 OFF
  if (GPIOWrite(MESSAGE, curr_state) == -1)
    return -1;
  fprintf(stdout, "화재 경보 시스템 시작\n");

  while (1)
  {
    curr_state = GPIORead(WARNING);

    if (prev_state == 0 && curr_state == 1)
    { // 버튼에서 짧게 누르고 손을 뗀 경우
      fprintf(stdout, "화재 발생 대피 바람\n");
      if (GPIOWrite(MESSAGE, curr_state) == -1)
        return -1;
    } else if (prev_state == 1 && curr_state == 0) {
      fprintf(stdout, "화재 경보 꺼짐 안심하세요\n");
      if (GPIOWrite(MESSAGE, curr_state) == -1)
        return -1;
    }
